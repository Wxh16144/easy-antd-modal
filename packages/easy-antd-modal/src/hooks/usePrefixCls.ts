...
function usePrefixCls(suffixCls: string, customPrefixCls?: string): string {
  ...
  const [[antdMajorVer]] = React.useState(() => version.split('.').map(Number));
  ...
  // v6 可以从 v5 平滑过渡
  const isModern = [5, 6].includes(antdMajorVer);

  /**
   * 5.x, 6.x 版本的 antd 会自动加上 easy- 前缀， 可以和 antd 的样式区分开。
   * 4.x 使用的是 less，不是很好处理，直接复用 antd 的样式即可。可以在稳定后直接删除对 4.x 的支持。
   * 二次开发约定 xxx-(antd 的组件 cls 逻辑)， 所以处理 4.x 时可以直接 pop() 获取最后一个即可。
   */
  const _prefixCls = isModern ? 'easy-' : '';
  let _suffixCls = isModern ? suffixCls : suffixCls.split('-').pop() ?? '';
